...
export async function loadResourceAllocationDetailPage(
  ensureQueryData: QueryClient["ensureQueryData"],
  id: string,
) {
  const allocationData = await ensureQueryData(getResourceAllocationDetailQueryOptions(id))

  return Promise.all([
    Promise.resolve(allocationData),
    ensureQueryData(getResourceAllocationPoolQueryOptions(allocationData.data.poolId)),
    ensureQueryData(getResourceAllocationProductQueryOptions(allocationData.data.productId)),

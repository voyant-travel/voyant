...
export function RatePlanInventoryOverridesTab({ propertyId }: Props) {
  ...
  const { data, isPending } = useRatePlanInventoryOverrides({
    limit: PAGE_SIZE,
    offset: pageIndex * PAGE_SIZE,
    enabled: !!propertyId,
  })
  const { remove } = useRatePlanInventoryOverrideMutation()
  const { baseUrl, fetcher } = useVoyantHospitalityContext()

  const rows = (data?.data ?? []) as RatePlanInventoryOverrideRecord[]

# @voyantjs/workflows-local-dashboard

The React app `voyant dev` serves at
`http://localhost:<port>/__voyant/workflows`. Embeds
`@voyantjs/workflows-react/ui` and points at the local miniflare
orchestrator.
...
```bash
VOYANT_WORKFLOWS_DASHBOARD_EMPTY_API=1 pnpm -F @voyantjs/workflows-local-dashboard dev:preview
```

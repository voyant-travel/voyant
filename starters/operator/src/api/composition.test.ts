...
describe("operator graph runtime composition", () => {
  ...
  it("binds selected package runtimes by declared ports instead of Commerce package ids", async () => {
    ...
    expect(Object.keys(ports)).toEqual(
      expect.arrayContaining([realtimeRuntimePort.id, storageMediaRuntimePort.id]),
    )

  ...
  for (const axis of INHERITING_AXES) {
    ...
    for (const ancestorPath of ancestors) {
      ...
      if (ancestor && ancestor[axis] !== undefined) {
        // biome-ignore lint/suspicious/noExplicitAny: union of FieldPolicy[axis] values across enum types
        ;(resolved as any)[axis] = ancestor[axis]
    ...
    if (!foundAncestorValue) {
      // biome-ignore lint/suspicious/noExplicitAny: union of FieldPolicy[axis] values across enum types
      ;(resolved as any)[axis] = INHERITING_DEFAULTS[axis]
    }
  }

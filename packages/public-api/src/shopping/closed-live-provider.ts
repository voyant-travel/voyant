...
import { createPublicApiShoppingLiveProvider } from "./live-provider.js"
import type {
  PublicApiCruiseSource,
  PublicApiCruiseSourceOffer,
...
import type { PublicApiResolvedScope } from "./schemas.js"

export interface ClosedPublicApiShoppingLiveProviderOptions {
 ...
 */
export function createClosedPublicApiShoppingLiveProvider(
  options: ClosedPublicApiShoppingLiveProviderOptions,
): PublicApiShoppingLiveProvider {
  const present =
    options.loadStayPresentation ?? options.catalogServices.presentAvailabilityCandidate
  const flights = options.flights

  return createPublicApiShoppingLiveProvider({
    ...(flights
      ? {
          flights: {
            async resolve(input) {
...
}): Promise<PublicApiCruiseSourceOffer | undefined> {
  const occupancy =

...
export class PublicApiShoppingScopeError extends Error {
  ...
  constructor(
    ...
    this.name = "PublicApiShoppingScopeError"
  }
}

export class PublicApiShoppingContinuationError extends Error {
  ...
  constructor() {
    ...
    this.name = "PublicApiShoppingContinuationError"
  }
}

export interface ManagedPublicApiShoppingRuntimeOptions {
  markets: PublicApiShoppingMarketProvider
  catalog: PublicApiShoppingCatalogProvider
  live: PublicApiShoppingLiveProvider
  references: PublicApiOpaqueReferenceIssuer
  quoteFx?: PresentationFxQuoter
  now?: () => Date
}

/** Managed OSS implementation of the provider-first shopping runtime. */
export function createManagedPublicApiShoppingRuntime(
  options: ManagedPublicApiShoppingRuntimeOptions,
...
async function resolveActiveScope(
  ...
  if (markets.length === 0) throw new PublicApiShoppingScopeError("market")
  ...
  if (!market) {
    if (requested.marketId) {
      throw new PublicApiShoppingScopeError("marketId", requested.marketId)
    }
    if (requested.locale && !availableLocale(availableLocales, requested.locale)) {
      throw new PublicApiShoppingScopeError("locale", requested.locale)
    }
    if (requestedCurrency && !availableCurrencies.includes(requestedCurrency)) {
      throw new PublicApiShoppingScopeError("currency", requestedCurrency)
    }
    throw new PublicApiShoppingScopeError("market")
  }
  ...
  if (!locale) throw new PublicApiShoppingScopeError("locale", requested.locale)
  if (!availableCurrencies.includes(currency)) {
    throw new PublicApiShoppingScopeError("currency", currency)
  }
...
function marketLocales(market: PublicApiActiveMarket): string[] {
...
function resolveSupportedLocale(
  market: PublicApiActiveMarket,
  requested: string,
): string | undefined {
  return availableLocale(marketLocales(market), requested)
}

type InspirationIntent = Extract<PublicApiShoppingIntent, { kind: "indexed-inspiration" }>
...
function publicCatalogItem(
  item: PublicApiCatalogSliceItem,
  itemRef: string,
...
async function resolveLiveContinuation(
  authority: PublicApiOpaqueReferenceIssuer,
  context: PublicApiShoppingContext,
  scope: PublicApiResolvedScope,
  intent: LiveIntent,
): Promise<{ state: PublicApiLiveContinuation; page: number } | undefined> {
  ...
  if (!resolved) throw new PublicApiShoppingContinuationError()
  ...
  if (
    ...
    throw new PublicApiShoppingContinuationError()
  }
...
function owner(context: PublicApiShoppingContext) {
  return {
...
function scopeBinding(scope: PublicApiResolvedScope) {
...
async function normalizeLive<T extends { nativePrice: { amount: string; currency: string } }>(
  page: PublicApiLiveSearchPage<T>,
  currency: string,
...
function coverage(page: PublicApiLiveSearchPage<unknown>, dropped: number) {

...
export {
  getPublicApiSlotResourceAvailability,
  getPublicApiSlotsResourceAvailability,
  type PublicApiSlotResourceAvailability,
} from "./service-boundary-resource-sql.js"

export type PublicApiSlotStatus = "open" | "closed" | "sold_out" | "cancelled"

export interface PublicApiSlotRow {
  id: string
  productId: string
...
export interface PublicApiProductPricingFacts {
  id: string
  sellCurrency: string
...
export interface PublicApiProductOptionFacts {
  id: string
  name: string
  description: string | null
}

export interface PublicApiOptionUnitFacts {
  id: string
  name: string
  unitType: string
  minAge: number | null
...
export interface PublicApiItineraryDay {
  id: string
  dayNumber: number
  title: string | null
  description: string | null
...
export interface PublicApiItineraryDayService {
  id: string
  dayId: string
  name: string
  description: string | null
...
export interface PublicApiItineraryDayMedia {
  id: string
  dayId: string | null
  url: string
  isCover: boolean
...
function mapSlot(row: PublicApiSlotDbRow): PublicApiSlotRow {
  return {
    id: row.id,
...
export async function listPublicApiSlots(
  db: PostgresJsDatabase,
  filters: {
    productId?: string
    slotId?: string
...
): Promise<PublicApiSlotRow[]> {
  const rows = await executeBoundaryRows<PublicApiSlotDbRow>(
    db,
...
export async function countPublicApiSlots(
  db: PostgresJsDatabase,
  filters: {
    productId?: string
    slotId?: string
  ...
  const rows = await executeBoundaryRows<{ value: number | string }>(
    db,
    // agent-quality: raw-sql reviewed -- owner: storefront; count uses the same public slot predicates as listPublicApiSlots.
    sql`
...
export async function loadProductPricingFacts(
  db: PostgresJsDatabase,
  productId: string,
): Promise<PublicApiProductPricingFacts | null> {
...
): Promise<PublicApiProductOptionFacts | null> {
...
export async function listOptionUnitFacts(
  db: PostgresJsDatabase,
  optionId: string,
): Promise<PublicApiOptionUnitFacts[]> {
...
): Promise<PublicApiItineraryDay[]> {
  return executeBoundaryRows<PublicApiItineraryDay>(
    db,
...
export async function listItineraryDayServices(
  ...
  dayIds: string[],
  languageTag?: string | null,
): Promise<PublicApiItineraryDayService[]> {
  const uniqueIds = [...new Set(dayIds)].filter(Boolean)
  if (uniqueIds.length === 0) return []

  return executeBoundaryRows<PublicApiItineraryDayService>(
    db,
...
): Promise<PublicApiItineraryDayMedia[]> {
  const uniqueDayIds = [...new Set(input.dayIds)].filter(Boolean)
  if (uniqueDayIds.length === 0) return []

  return executeBoundaryRows<PublicApiItineraryDayMedia>(
    db,

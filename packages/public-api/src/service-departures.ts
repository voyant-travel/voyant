...
export async function getPublicApiDeparture(db: PostgresJsDatabase, departureId: string) {
  ...
  const [meetingPointByProduct, defaultItineraryByProduct, resourceAvailability] =
    await Promise.all([
      ...
      getPublicApiSlotsResourceAvailability(db, [slot.id]),
    ])
...
export async function listPublicApiProductDepartures(
  db: PostgresJsDatabase,
  productId: string,
...
export async function getPublicApiProductAvailabilitySummary(
  db: PostgresJsDatabase,
  productId: string,
...
export async function getPublicApiDepartureItinerary(
  db: PostgresJsDatabase,

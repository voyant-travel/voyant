...
describe("createPublicApiRoutes", () => {
  ...
  it("fails closed when the publication provider is unavailable", async () => {
    const app = createActivePublicApiApp()
    app.route("/", createPublicApiRoutes())
  ...
  it("serves a storefront whose channel context came from the implicit Direct default", async () => {
    ...
    app.route("/", createPublicApiRoutes({ publication: { isProductPublished } }))
  ...
  it("returns normalized storefront settings", async () => {
    const app = createActivePublicApiApp().route(
      "/",
      createPublicApiRoutes({
        settings: {
          support: {
  ...
  it("fills missing storefront settings with stable defaults", async () => {
    const app = createActivePublicApiApp().route("/", createPublicApiRoutes())

...
import { createClosedPublicApiShoppingLiveProvider } from "../../src/shopping/closed-live-provider.js"
import { createManagedPublicApiShoppingRuntime } from "../../src/shopping/managed-runtime.js"
import type { PublicApiOpaqueReferenceIssuer } from "../../src/shopping/runtime-port.js"

const context = { channelId: "ch_1" }
const scope = {
  marketId: "market_ro",
  locale: "ro-RO",
  currency: "EUR",
...
describe("closed Storefront live provider", () => {
  it("fans out only admitted stay-capable adapters and the owned accommodations handler", async () => {
    ...
    const provider = createClosedPublicApiShoppingLiveProvider({
  ...
  it("keeps exact offer ownership opaque across partial multi-source flight search", async () => {
    ...
    const live = createClosedPublicApiShoppingLiveProvider({
      primitives: primitives(),
      catalogServices: {} as never,
      markets: markets(),
      flights: { listAdmittedShoppingSources },
    })
    const issued: Array<Parameters<PublicApiOpaqueReferenceIssuer["issue"]>[0]> = []
    let sequence = 0
    const runtime = createManagedPublicApiShoppingRuntime({
      markets: markets(),

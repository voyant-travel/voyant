...
import {
  createManagedPublicApiShoppingRuntime,
  inspirationCatalogSlice,
  type PublicApiInternalFlightOffer,
  type PublicApiInternalPackageOffer,
  type PublicApiShoppingContext,
  PublicApiShoppingScopeError,
...
describe("managed storefront shopping scope", () => {
  it("uses active trusted channel config, returns defaults/pickers, and passes no PII", async () => {
    ...
    const runtime = createManagedPublicApiShoppingRuntime(deps)
  ...
  it.each([
    ...
    const runtime = createManagedPublicApiShoppingRuntime(dependencies())
...
describe("managed live shopping", () => {
  ...
  function flight(amount: string, currency: string, marker: string): PublicApiInternalFlightOffer {
    return {
  ...
  it("seals provider-native package pricing for Trip freeze without trusting presentation FX", async () => {
    ...
    } satisfies PublicApiInternalPackageOffer

...
import { createPublicApiAdminRoutes } from "../../src/routes-admin.js"
import type { PublicApiSettingsInput } from "../../src/validation.js"

function createTestApp(options: Parameters<typeof createPublicApiAdminRoutes>[0]) {
  const app = new Hono()
  app.onError(handleApiError)
  return app.route("/", createPublicApiAdminRoutes(options))
}

describe("createPublicApiAdminRoutes", () => {
  ...
  it("patches each settings section and preserves omitted values", async () => {
    let settings: PublicApiSettingsInput = {

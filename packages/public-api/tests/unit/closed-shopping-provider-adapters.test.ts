...
import { createClosedPublicApiShoppingAdapters } from "../../src/shopping/closed-provider-adapters.js"
import { PublicApiShoppingUnavailableError } from "../../src/shopping/runtime.js"

const storefront = { channelId: "channel_web" }
...
function adapters(
  ...
  return createClosedPublicApiShoppingAdapters({
    primitives: {
    ...
    isActivePublicApiChannel: vi.fn(async (_db, context) =>
...
describe("closed storefront shopping market adapter", () => {
  ...
  it("keeps an active channel isolated to its trusted context", async () => {
    ...
    await expect(
      provider.listActiveMarkets({ ...storefront, channelId: "chan_other" }),
    ).rejects.toBeInstanceOf(PublicApiShoppingUnavailableError)
  })
...
describe("closed storefront shopping catalog adapter", () => {
  ...
  it("fails unavailable when the selected Catalog runtime has no indexer", async () => {
    const noIndexerProvider = createClosedPublicApiShoppingAdapters({
      primitives: {
        env: vi.fn(() => ({})),
      ...
      isActivePublicApiChannel: async () => true,
    }).catalog
    await expect(

...
import { type Trip, TripComposerInvariantError, type TripListResult } from "./service-types.js"
...
export async function reorderComponents(
  ...
  for (const row of rows) {
    ...
    if (row.status === "removed") {
      throw new TripComposerInvariantError(`Trip component ${row.id} is removed`)
    }
  }
...
async function getTripComponentOrThrow(db: AnyDrizzleDb, id: string): Promise<TripComponent> {
  ...
  if (!component) throw new TripComposerInvariantError(`Trip component ${id} was not found`)
  return component
}

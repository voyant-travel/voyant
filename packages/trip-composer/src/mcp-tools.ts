...
export interface TripComposerMcpServices {
...
type TripComposerMcpContext = McpToolContext & {
  tripComposer?: TripComposerMcpServices
}
...
export const createTripTool: McpToolDefinition<CreateTripArgs, McpToolResult> = {
  ...
  async handler(args, rawContext) {
    const context = rawContext as TripComposerMcpContext
    const composer = requireTripComposer(context)
  ...
  async handler(args, rawContext) {
    ...
    if (args.removeComponentIds.length > 0 && !composer.removeComponent) {
      throw new McpToolError(
        "Trip composer MCP service removeComponent is not configured.",
        "MISSING_SERVICE",
        { service: "tripComposer.removeComponent" },
      )
    }
...
export const priceTripTool: McpToolDefinition<PriceTripArgs, McpToolResult> = {
  name: "price_trip",
  description:
    "Price a deterministic trip through the trip composer. Returns aggregate totals, " +
...
export const reserveTripTool: McpToolDefinition<ReserveTripArgs, McpToolResult> = {
  name: "reserve_trip",
  description:
    "Reserve a priced trip through deterministic trip composer services. " +
...
export const tripComposerMcpTools = [
  createTripTool,
  reviseTripTool,
  priceTripTool,
  reserveTripTool,
...
function requireTripComposer(context: TripComposerMcpContext): TripComposerMcpServices {
  if (!context.tripComposer) {
    throw new McpToolError(
      "MCP tool requires trip composer services to be wired into the context.",
      "MISSING_SERVICE",
      { service: "tripComposer" },
    )
  }
  return context.tripComposer
}

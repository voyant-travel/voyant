...
async function writeCacheRow(
  ...
  const freshUntil =
    toDateOrNull(result.fresh_until) ?? new Date(now.getTime() + CHARTERS_DEFAULT_TTL_MS)
...
export const invalidateCharterContentOnDrift: InvalidateOnDrift = createInvalidateOnDrift(
  chartersSourcedContentTable,

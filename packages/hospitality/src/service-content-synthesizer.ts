/**
 * Hospitality content synthesizer — fallback for thin adapters that
 * declare `supportsContentFetch: false`.
 *
 * Produces the most complete `HospitalityContent` blob we can
...
export function synthesizeHospitalityContent(
  scope: { locale: string },
  options: SynthesizeHospitalityContentOptions,
): SynthesizedHospitalityContent {
  ...
  const baseContent: HospitalityContent = {
    hotel,
    room_types: [],
    rate_plans: [],
    meal_plans: [],
  ...
  if (options.overlays && options.overlays.length > 0) {
    const result = mergeOverlaysIntoContent(baseContent, options.overlays, {
      validate(p) {
        const r = hospitalityContentSchema.safeParse(p)
        return r.success
          ? { valid: true }
          : { valid: false, reason: r.error.issues[0]?.message ?? "invalid" }
      },
    })
    merged = hospitalityContentSchema.parse(result)
  }

  return {
    content: merged,
    content_schema_version: HOSPITALITY_CONTENT_SCHEMA_VERSION,
...
export async function synthesizeHospitalityContentFromDb(
  db: AnyDrizzleDb,
  scope: { locale: string },
  provenance: Extract<ProvenanceReadResult, { kind: "sourced" }>,
): Promise<SynthesizedHospitalityContent> {
  const entityId = entityIdFromProvenance(provenance)
  const overlays = await fetchOverlaysForEntity(db, "hospitality", entityId)
  return synthesizeHospitalityContent(scope, {
    provenance,
...
function pickAmenities(projection: Record<string, unknown>): HospitalityContent["amenities"] {
  ...
  const out: HospitalityContent["amenities"] = []
...
function pickPolicies(projection: Record<string, unknown>): HospitalityContent["policies"] {
  const out: HospitalityContent["policies"] = []

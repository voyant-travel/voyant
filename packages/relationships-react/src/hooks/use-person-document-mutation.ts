...
export function usePersonDocumentMutation(personId: string | undefined) {
  ...
  const invalidate = () => {
    if (personId) {
      void queryClient.invalidateQueries({
        queryKey: [...relationshipsQueryKeys.person(personId), "documents"],
      })
    }
  }
  ...
  const update = useMutation({
    ...
    onSuccess: (data) => {
      invalidate()
      queryClient.setQueryData(relationshipsQueryKeys.personDocument(data.id), data)
    },
  })
  ...
  const remove = useMutation({
    ...
    onSuccess: (_data, id) => {
      invalidate()
      queryClient.removeQueries({ queryKey: relationshipsQueryKeys.personDocument(id) })
    },
  })

...
export interface TripsMcpServices {
...
type TripsMcpContext = McpToolContext & {
  trips?: TripsMcpServices
}
...
export const createTripTool: McpToolDefinition<CreateTripArgs, McpToolResult> = {
  ...
  async handler(args, rawContext) {
    const context = rawContext as TripsMcpContext
    const composer = requireTrips(context)
  ...
  async handler(args, rawContext) {
    ...
    if (args.removeComponentIds.length > 0 && !composer.removeComponent) {
      throw new McpToolError(
        "Trips MCP service removeComponent is not configured.",
        "MISSING_SERVICE",
...
export const priceTripTool: McpToolDefinition<PriceTripArgs, McpToolResult> = {
  name: "price_trip",
  description:
    "Price a deterministic trip through the trips. Returns aggregate totals, " +
...
export const reserveTripTool: McpToolDefinition<ReserveTripArgs, McpToolResult> = {
  name: "reserve_trip",
  description:
    "Reserve a priced trip through deterministic trips services. " +
...
function requireTrips(context: TripsMcpContext): TripsMcpServices {
  if (!context.trips) {
    throw new McpToolError(
      "MCP tool requires trips services to be wired into the context.",
      "MISSING_SERVICE",
      { service: "trips" },
    )
  }

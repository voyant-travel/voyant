import { type RatePlanRecord, useRatePlan, useRatePlans } from "@voyantjs/hospitality-react"
import {
  Combobox,
  ComboboxCollection,
  ComboboxContent,
...
export function RatePlanCombobox({ propertyId, value, onChange, placeholder, disabled }: Props) {
  ...
  const items = React.useMemo(() => {
    const map = new Map<string, RatePlanRecord>()
  ...
  return (
    ...
    >
      <ComboboxInput
        placeholder={placeholder ?? messages.comboboxes.ratePlan.placeholder}
        showClear={!!value}
      />
      <ComboboxContent>
        <ComboboxEmpty>
          {listQuery.isPending || selectedQuery.isPending
            ...
            : messages.comboboxes.ratePlan.empty}
        </ComboboxEmpty>
        <ComboboxList>
          <ComboboxCollection>
            {(id) => {

...
```bash
pnpm add @voyantjs/charters-ui @voyantjs/charters-react @voyantjs/ui @tanstack/react-query react react-dom
```

`@voyantjs/ui` provides the design-system primitives. `@voyantjs/charters-react` provides the data-layer hooks. Both are required peers.

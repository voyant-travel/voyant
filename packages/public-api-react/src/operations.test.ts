...
import { getAdminPublicApiSettings, updateAdminPublicApiSettings } from "./operations.js"

const publicApiSettingsResponse = {
  data: {
    support: {
      email: null,
      phone: null,
...
describe("admin storefront settings operations", () => {
  it("reads admin storefront settings from the admin endpoint", async () => {
    const fetcher = vi
      .fn<VoyantFetcher>()
      .mockResolvedValueOnce(Response.json(publicApiSettingsResponse))

    await expect(
      getAdminPublicApiSettings({ baseUrl: "https://operator.example/api", fetcher }),
    ).resolves.toEqual(publicApiSettingsResponse)

    expect(fetcher).toHaveBeenCalledWith(
      "https://operator.example/api/v1/admin/public-api/settings",
  ...
  it("validates and sends settings updates as PATCH requests", async () => {
    ...
    await updateAdminPublicApiSettings(

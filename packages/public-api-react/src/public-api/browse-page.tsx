...
import { PublicApiLink, usePublicApiUi } from "./context.js"
 ...
 * `publicApiCustomerBookableProductVerticals`. Deriving the accepted verticals
 ...
 */
export function PublicApiBrowsePage({
  search,
}: {
  search: z.infer<typeof shopSearchSchema>
}): React.ReactElement {
  const { messages, navigate, scope } = usePublicApiUi()
...
function SearchUnavailable(): React.ReactElement {
  const t = usePublicApiUi().messages.shop
  return (
    <Card>
      <CardHeader>
        ...
        return (
          <Card key={hit.id}>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              {(() => {
                const route = getPublicApiCustomerProductDetailRoute(vertical, hit.id)
                return route ? (
                  <PublicApiLink

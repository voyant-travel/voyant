...
import { ACCOMMODATION_CONTENT_SCHEMA_VERSION } from "../../src/content-shape.js"
import { synthesizeAccommodationContent } from "../../src/service-content-synthesizer.js"
...
describe("synthesizeAccommodationContent — typed empty states", () => {
  it("produces a minimal-but-valid payload from an almost-empty projection", () => {
    const result = synthesizeAccommodationContent(
      { locale: "en-GB" },
      { provenance: makeProvenance({ id: "hrmt_abc" }) },
    )
    expect(result.content_schema_version).toBe(ACCOMMODATION_CONTENT_SCHEMA_VERSION)

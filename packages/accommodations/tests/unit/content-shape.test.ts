...
const baseContent: AccommodationContent = accommodationContentSchema.parse({
  hotel: {
    id: "hrmt_abc",
    name: "Hotel Sample",
    star_rating: 4,
...
describe("ACCOMMODATION_CONTENT_SCHEMA_VERSION", () => {
  it("is the accommodations/v1 stable identifier", () => {
    expect(ACCOMMODATION_CONTENT_SCHEMA_VERSION).toBe("accommodations/v1")
  })
})

describe("validateAccommodationContent", () => {
  it("accepts a minimal valid payload (only required hotel.{id,name})", () => {
    expect(validateAccommodationContent({ hotel: { id: "hrmt_abc", name: "X" } }).valid).toBe(true)
  })

  it("accepts a full valid payload", () => {
    expect(validateAccommodationContent(baseContent).valid).toBe(true)
  })

  it("rejects missing hotel.id / hotel.name", () => {
    expect(validateAccommodationContent({ hotel: {} }).valid).toBe(false)
    expect(validateAccommodationContent({}).valid).toBe(false)
  })
...
describe("mergeOverlaysIntoAccommodationContent", () => {
  it("applies a top-level hotel field overlay", () => {
    const merged = mergeOverlaysIntoAccommodationContent(baseContent, [
  ...
  it("does not mutate the input payload", () => {
    ...
    mergeOverlaysIntoAccommodationContent(baseContent, [

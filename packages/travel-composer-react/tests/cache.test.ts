...
import { travelComposerQueryKeys } from "../src/query-keys.js"

describe("travel composer react cache helpers", () => {
  it("writes priced trip results into trip, component, and pricing caches", () => {
    ...
    expect(queryClient.getQueryData(travelComposerQueryKeys.trip("trip_123"))).toEqual({
      envelope: { id: "trip_123" },
      components: [{ id: "trcp_1" }],
    })
    expect(queryClient.getQueryData(travelComposerQueryKeys.components("trip_123"))).toEqual([
      { id: "trcp_1" },
    ])
    expect(queryClient.getQueryData(travelComposerQueryKeys.pricing("trip_123"))).toEqual({
  ...
  it("writes checkout targets into the checkout cache", () => {
    ...
    expect(queryClient.getQueryData(travelComposerQueryKeys.checkout("trip_123"))).toEqual({
      envelopeId: "trip_123",

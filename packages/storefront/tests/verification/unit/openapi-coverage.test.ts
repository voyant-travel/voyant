...
import { createStorefrontVerificationApiModule } from "../../../src/verification/index.js"
...
describe("storefront verification OpenAPI coverage", () => {
  it("documents every public route it serves, and serves every one it documents", () => {
    const apiModule = createStorefrontVerificationApiModule()
    ...
    const committed = JSON.parse(
      readFileSync(
        fileURLToPath(
          new URL("../../../openapi/storefront/storefront-verification.json", import.meta.url),
        ),
        "utf8",
      ),
    )
    ...
    const diff = diffOpenApiCoverage({
      runtime,
      committed,
      prefix: "/v1/public/storefront-verification",
    })

...
Make a **single canonical product-detail page** that lives in `@voyantjs/products-ui`
...
- `@voyantjs/products-ui` ships an **old `ProductDetailPage`** (`packages/products-ui/src/components/product-detail-page.tsx`) that is **not imported by any template** — effectively dead.
...
`packages/products-ui/src/components/product-detail/`
...
Verify: `grep -rln '@voyantjs/admin\|@/lib/admin-i18n\|@/lib/zod-resolver\|@/lib/timezone-options' packages/products-ui/src/components/product-detail/` → should be **0**.
...
### 5.5 Dependencies to add to `packages/products-ui/package.json`
...
- Decide the canonical export. Suggested: export `ProductDetailPage` + `ProductDetailHostProvider`
  (and the host types) from `product-detail/`. Update `packages/products-ui/src/index.ts` (or add a
  `./components/product-detail` subpath export in `package.json`).
- The OLD `packages/products-ui/src/components/product-detail-page.tsx` is unused — can be deleted
...
### 5.8 Typecheck / lint
- `pnpm -F @voyantjs/products-ui typecheck` (expect a long fix tail on first run), then

...
- 61673e1: Closes #505: introduce a `pricing.rule.changed` event surface so the catalog bridge reindexes products when option-price rules are mutated.
...
- 24a121e: Resolve option price rules by date via priceSchedule + RRULE (#465).
...
- b218885: Add global created-time indexes for pricing child-list admin endpoints that can
  paginate without a parent filter.
- b218885: Add composite list indexes for pricing admin queries.

...
export const PAYMENT_CONFIRMATION_MARKER_PREFIX = "__payment_confirmation__:"
...
export async function generateBookingContractOnConfirmation(
  ...
  if (result) {
    await promotePaidAcceptedBookingContract(input.db, result.contractId, input.eventBus)
  }
  return result
    ? {
        status: "generated",
...
> {
  ...
  const paymentConfirmation =
    priorMetadata.paymentConfirmation ??
    bookingPaymentConfirmationMetadata(booking.internalNotes) ??
    undefined
  const metadata = {
    ...priorMetadata,
    autoGenerated: true,
...
export function bookingPaymentConfirmationMetadata(internalNotes: string | null): {
  paymentSessionId: string
  confirmedAt: string
} | null {
  if (!internalNotes) return null
  for (const line of internalNotes.split("\n")) {
    if (!line.startsWith(PAYMENT_CONFIRMATION_MARKER_PREFIX)) continue
    try {
      const value = record(JSON.parse(line.slice(PAYMENT_CONFIRMATION_MARKER_PREFIX.length)))
      if (
        typeof value.paymentSessionId === "string" &&
        value.paymentSessionId.length > 0 &&
      ...
      // Ignore malformed markers and keep looking for a valid checkpoint.
    }
  }
  return null
}

async function promotePaidAcceptedBookingContract(
  db: PostgresJsDatabase,
  contractId: string,
  eventBus?: EventBus,
): Promise<void> {
  const [contract] = await db.select().from(contracts).where(eq(contracts.id, contractId)).limit(1)
  if (!contract?.bookingId || contract.status !== "draft") return
  const metadata = record(contract.metadata)
  if (!metadata.acceptance || !metadata.paymentConfirmation || !metadata.bookingContractWorkflow) {
    return
  }
  if ((await contractsService.listSignatures(db, contractId)).length > 0) return
  const booking = await bookingsService.getBookingById(db, contract.bookingId)
  if (!booking) return

  const issued = await contractsService.issueContract(
    db,
    contractId,
    { eventBus },
  ...
  const sent = await contractsService.sendContract(db, contractId, { eventBus }, undefined, {
    allowManagedBookingContractWorkflow: true,
  })
  ...
  const signed = await contractsService.signContract(
    db,
    contractId,
    {
      signerName,
...
export async function bookingContractAcceptanceMetadata(input: {
  ...
  if (!input.internalNotes) return null
  for (const line of input.internalNotes.split("\n")) {
    if (!line.startsWith(CONTRACT_ACCEPTANCE_MARKER_PREFIX)) continue
    try {
      const value = record(JSON.parse(line.slice(CONTRACT_ACCEPTANCE_MARKER_PREFIX.length)))
      if (typeof value.acceptedAt !== "string" || Number.isNaN(Date.parse(value.acceptedAt))) {
        continue
      }
      if (
        value.templateId !== input.templateId ||
        value.templateVersionId !== input.templateVersionId ||
        !isBookingContractAcceptanceContentDigest(value.contentDigest)
      ) {
        continue
      }

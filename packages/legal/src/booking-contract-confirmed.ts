...
> {
  ...
  const pendingAcceptance = bookingContractAcceptanceMetadata({
...
export function bookingContractAcceptanceMetadata(input: {
  ...
  for (const line of input.internalNotes.split("\n")) {
    ...
    try {
      ...
      return {
        ...
        renderedHtmlLength: input.renderedHtmlLength,
      }
    } catch {

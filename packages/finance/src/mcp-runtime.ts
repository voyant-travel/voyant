 ...
 * command." The reason travelled in `meta`, where the model does not look, and
 * the message named none of it.
 *
 * Measured over ten runs against the real graph, issuing a proforma succeeded
 * 0/10. The agent performed the whole protocol correctly — preview, request
 * approval, approve, issue — and then had no way to learn which of fourteen
 * things had gone wrong, so it could not take a next step. Every other error in
 * this area names its cause and its fix; this one now does too.
 */

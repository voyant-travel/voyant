...
Some components couple to TanStack Router or template-local helpers and remain available only via the shadcn registry: `contract-detail-page`, `policy-detail-page`, `template-detail-page`. Import via `npx shadcn add @voyant/<component>` and customize per-project.

...
export async function emitFirstResponseOverdueEvents(
  ...
  // Claim the derived SLA edge in an append-only ledger before enqueueing, in
  // the same transaction. Inquiry itself stores timestamps, never "overdue".
  const due = await db

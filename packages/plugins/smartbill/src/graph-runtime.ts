...
export interface SmartbillRuntimeConfig {
  username: string
  apiToken: string
...
export interface SmartbillRuntimeHost {
  resolveDatabase: (bindings: unknown) => PostgresJsDatabase
  resolveConfig: (bindings: unknown) => SmartbillRuntimeConfig | null
  resolveDocumentStorage?: (bindings: unknown) => StorageProvider | null
  logger?: Pick<Console, "info" | "warn" | "error">
}

export interface SmartbillOwnedSubscriberRuntime {
  bootstrap: () => Promise<void>
  invoiceIssued: (envelope: EventEnvelope) => Promise<void>
  proformaIssued: (envelope: EventEnvelope) => Promise<void>
  paymentRecorded: (envelope: EventEnvelope) => Promise<void>
}

export interface SmartbillRuntimeDependencies {
  client?: SmartbillClientApi
  fetch?: typeof fetch
}

/** Package-owned adapter from the selected graph's typed Node host port. */
export const createSmartbillVoyantRuntime = defineGraphRuntimeFactory<ApiModule>(
  async ({ getPort }) => {
...
export type SmartbillExternalRefLike = {
  provider: string
...
export type SmartbillEstimateReference = {
  seriesName: string
  number: string
}
...
export type SmartbillPaymentType =
  | "Card"
  | "CEC"
  | "Bilet ordin"
  | "Ordin plata"
...
export type SmartbillPaymentBody = {
  companyVatCode: string
  issueDate: string
  currency: string
  value: number
...
export function createSmartbillOwnedSubscriberRuntime(
  host: SmartbillRuntimeHost,
  bindings: unknown,
  dependencies: SmartbillRuntimeDependencies = {},
): SmartbillOwnedSubscriberRuntime | null {
  const config = host.resolveConfig(bindings)
  if (!config) return null
  const runtime = createRuntime(host, bindings, config, dependencies)

  return {
    bootstrap: () => ensureSmartbillInvoiceNumberSeries(runtime),
    invoiceIssued: ({ data }) =>
      syncIssuedInvoice(runtime, data as InvoiceIssuedPayload, "invoice"),
    proformaIssued: ({ data }) =>
      syncIssuedInvoice(runtime, data as InvoiceIssuedPayload, "proforma"),
    paymentRecorded: ({ data }) =>
      syncRecordedInvoicePayment(runtime, data as InvoicePaymentRecordedEvent),
  }
}

export function createSmartbillSettlementPollers(
  config: SmartbillRuntimeConfig | null,
...
async function ensureSmartbillInvoiceNumberSeries(runtime: SmartbillRuntime) {
  try {
    await financeService.ensureExternalInvoiceNumberSeries(runtime.db, [
      {
        provider: "smartbill",
        scope: "invoice",
    ...
    runtime.logger.warn("[smartbill] invoice number series bootstrap failed", error)
  }
}
...
function createRuntime(
  host: SmartbillRuntimeHost,
  bindings: unknown,
  config: SmartbillRuntimeConfig,
  dependencies: SmartbillRuntimeDependencies,
): SmartbillRuntime {
  const invoiceSeriesName = config.invoiceSeriesName ?? config.seriesName
  const proformaSeriesName = config.proformaSeriesName ?? invoiceSeriesName
  return {
    ...config,
    invoiceSeriesName,
    proformaSeriesName,
    art311SpecialRegime: config.art311SpecialRegime ?? false,
    db: host.resolveDatabase(bindings),
    documentStorage: host.resolveDocumentStorage?.(bindings) ?? null,
    logger: host.logger ?? console,
    fetch: dependencies.fetch ?? globalThis.fetch,
    client:
      (dependencies.client as ReturnType<typeof createSmartbillClient> | undefined) ??
      createSmartbillClient(config),
  }
}
...
async function syncIssuedInvoiceWithDb(
  ...
  try {
    ...
    if (result.number && !result.errorText) {
      try {
        await fetchAndStoreSmartbillPdf(db, runtime, {
          invoiceId,
          documentType,
    ...
    runtime.logger.error(`[smartbill] failed to create ${documentType} for ${invoiceId}`, error)
    await financeService.registerInvoiceExternalRef(db, invoiceId, {
      provider: "smartbill",
      status: "error",
      syncedAt: new Date().toISOString(),
...
async function syncRecordedInvoicePayment(
  ...
  await syncRecordedInvoicePaymentWithDb(runtime.db, runtime, payload)
}
...
async function createSmartbillPayment(runtime: SmartbillRuntime, body: SmartbillPaymentBody) {
  ...
  const response = await runtime.fetch(`${apiUrl}/payment`, {
    method: "POST",
    headers: {

...
/**
 * `mapLineItems` takes only the options that shape a product line
 * (`isTaxIncluded`, `measuringUnitName`); company and series belong to the
 * invoice-body mappers.
 */
...
describe("mapLineItems", () => {
  ...
  it("returns empty array when no lineItems", () => {
    expect(mapLineItems(event(), lineItemOptions)).toEqual([])
  })

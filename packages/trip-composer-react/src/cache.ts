...
export function writeTripCache(queryClient: QueryClient, result: TripLikeResult) {
  void queryClient.invalidateQueries({ queryKey: tripComposerQueryKeys.trips() })
  queryClient.setQueryData(tripComposerQueryKeys.trip(result.envelope.id), {
    envelope: result.envelope,
    components: result.components,
  })
  queryClient.setQueryData(tripComposerQueryKeys.components(result.envelope.id), result.components)
}

export function writePriceTripCache(queryClient: QueryClient, result: PriceTripResult) {
  writeTripCache(queryClient, result)
  queryClient.setQueryData(tripComposerQueryKeys.pricing(result.envelope.id), result.pricing)
}
...
export function writeTripCheckoutCache(queryClient: QueryClient, result: StartCheckoutResult) {
  writeTripCache(queryClient, result)
  queryClient.setQueryData(tripComposerQueryKeys.checkout(result.envelope.id), result.target)
}

// agent-quality: file-size exception -- owner: trip-composer-react; existing UI surface stays co-located until a dedicated split preserves behavior and tests.
"use client"

import { useQuery } from "@tanstack/react-query"
import { useOperatorAdminMessages as useAdminMessages, useAdminNavigate } from "@voyantjs/admin"
import { formatMessage } from "@voyantjs/i18n"
import type { Trip, TripEnvelopeStatus, TripsListSortField } from "@voyantjs/trip-composer"
...
/**
 * Initial list parameters mirrored by the `trip-composer-index`
  ...
  return (
    <>
      {Array.from({ length: rows }).map((_, idx) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: skeleton placeholders are stable -- owner: trip-composer-react; existing suppression is intentional pending typed cleanup.

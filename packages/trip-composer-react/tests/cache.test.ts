...
import { tripComposerQueryKeys } from "../src/query-keys.js"

describe("trip composer react cache helpers", () => {
  it("writes priced trip results into trip, component, and pricing caches", () => {
    ...
    expect(queryClient.getQueryData(tripComposerQueryKeys.trip("trip_123"))).toEqual({
      envelope: { id: "trip_123" },
      components: [{ id: "trcp_1" }],
    })
    expect(queryClient.getQueryData(tripComposerQueryKeys.components("trip_123"))).toEqual([
      { id: "trcp_1" },
    ])
    expect(queryClient.getQueryData(tripComposerQueryKeys.pricing("trip_123"))).toEqual({
  ...
  it("writes checkout targets into the checkout cache", () => {
    ...
    expect(queryClient.getQueryData(tripComposerQueryKeys.checkout("trip_123"))).toEqual({
      envelopeId: "trip_123",

...
describe("trip composer react operations", () => {
  it("reads a trip from the admin composer route", async () => {
    ...
    expect(calls[0]?.url).toBe("https://app.example/v1/admin/trip-composer/trips/trip_123")
  })
  ...
  it("lists trips with query params", async () => {
    ...
    expect(calls[0]?.url).toBe(
      "https://app.example/v1/admin/trip-composer/trips?status=reserved&search=istanbul&accommodationId=acc_123&limit=25&offset=50",
    )
  })
  ...
  it("can target the public composer route", async () => {
    ...
    expect(calls.map((call) => call.url)).toEqual([
      "https://app.example/v1/public/trip-composer/trips",
      "https://app.example/v1/public/trip-composer/trips/trip_123",
      "https://app.example/v1/public/trip-composer/trips/trip_123/components",
    ])
  })
  ...
  it("posts price, reserve, and checkout bodies without duplicating envelopeId", async () => {
    ...
    expect(calls.map((call) => call.url)).toEqual([
      "https://app.example/v1/admin/trip-composer/trips/trip_123/price",
      "https://app.example/v1/admin/trip-composer/trips/trip_123/reserve",
      "https://app.example/v1/admin/trip-composer/trips/trip_123/checkout",
    ])
  ...
  it("reads and creates trip snapshots from the admin composer route", async () => {
    ...
    expect(calls.map((call) => call.url)).toEqual([
      "https://app.example/v1/admin/trip-composer/trips/trip_123/snapshots",
      "https://app.example/v1/admin/trip-composer/trip-snapshots/trsn_123",
      "https://app.example/v1/admin/trip-composer/trips/trip_123/snapshots",
    ])
  ...
  it("freezes and applies a trip snapshot to a quote version through the operator bridge", async () => {
    ...
    expect(calls[0]?.url).toBe(
      "https://app.example/v1/admin/trip-composer/trips/trip_123/quote-versions/qver_123/snapshot",
    )
  ...
  it("posts cancellation preview and selected-component cancel bodies", async () => {
    ...
    expect(calls.map((call) => call.url)).toEqual([
      "https://app.example/v1/admin/trip-composer/trips/trip_123/cancellation-preview",
      "https://app.example/v1/admin/trip-composer/trips/trip_123/cancel-components",
    ])
    expect(calls.map((call) => JSON.parse(String(call.init?.body)))).toEqual([

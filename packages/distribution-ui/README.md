...
```bash
pnpm add @voyantjs/distribution-ui @voyantjs/distribution-react @voyantjs/voyant-ui @tanstack/react-query react react-dom
```

`@voyantjs/voyant-ui` provides the design-system primitives. `@voyantjs/distribution-react` provides the data-layer hooks. Both are required peers.

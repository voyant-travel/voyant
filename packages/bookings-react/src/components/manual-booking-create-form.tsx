...
export function ManualBookingCreateForm({
  ...
  const travelerPricingCategoryQuantities = React.useMemo(() => {
    ...
    for (const traveler of travelers.travelers) {
      ...
      const unitQuantities = quantities[traveler.inventoryUnitId] ?? {}
      unitQuantities[pricingCategoryId] = (unitQuantities[pricingCategoryId] ?? 0) + 1
      quantities[traveler.inventoryUnitId] = unitQuantities
    }
    return quantities
  ...
  const quoteTotalAmountCents =
    quote.data?.available === false ? null : (quote.data?.pricing?.total ?? null)
  ...
  const manualOverrideRequiresReason = Boolean(pricing?.requiresReason)
  const promoFeedback = promotionCode.trim()
    ? quote.isQuoting
      ? copy.promotion.checking
      : quote.data?.available === false
        ? quote.data.invalidReason || copy.promotion.invalid
        : quote.data?.pricing
  ...
  const handleSubmit = async () => {
    ...
    if (quote.data?.available === false) {
      setError(quote.data.invalidReason || copy.promotion.invalid)
      return
    }
    ...
    const booking = {
      ...
      sellAmountCentsOverride: pricing?.isManualOverride
        ? resolvedPricing.confirmedAmountCents
        : null,
        ...
        {product.productId && slotId ? (
          <FieldSet className="gap-3 rounded-md border p-3">
            ...
            <Field className="gap-2">
              ...
              {promoFeedback ? (
                <FieldDescription
                  className={quote.data?.available === false ? "text-destructive" : undefined}
                >
                  {promoFeedback}
                </FieldDescription>
              ) : null}

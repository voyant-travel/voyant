import type { BookingsUiJourneyMessages } from "./messages-journey.js"

export const bookingsUiEnJourney = {
  bookingJourney: {
    steps: {
      departure: "Departure",
      billing: "Billing",
      travelers: "Travelers",
      options: "Options",
      accommodation: "Accommodation",
      addons: "Extras",
      ancillaries: "Partner offers",
      payment: "Payment",
      documents: "Documents & notes",
      review: "Review",
      billingAndContact: "Billing & contact",
      reviewAndConfirm: "Review & confirm",
    },
    navigation: {
      back: "Back",
      next: "Next",
      checking: "Checking...",
      continue: "Continue",
      edit: "Edit",
      done: "Done",
      cancel: "Cancel",
    },
    values: {
      noValue: "—",
      notSet: "Not set",
      none: "None",
      selectPlaceholder: "Select...",
    },
    validation: {
      completeStepBeforeContinuing: "Complete this step before continuing.",
      unableToContinue: "Unable to continue. Please try again.",
      quoteFailed: "We couldn't refresh live pricing. Check your connection and try again.",
      retryQuote: "Retry pricing",
      quoteUnavailable:
        "Pricing isn't confirmed yet — retry pricing above before confirming your booking.",
      pricingUnavailable:
        "This selection can't be priced right now. Please adjust your choice before confirming your booking.",
      checkoutFailed: "We couldn't complete your booking. Please try again.",
      paidPaymentDateRequired: "Payment date is required when Already paid is checked.",
      addAtLeastTravelers: "Add at least {count} traveler{plural} to continue.",
      maxTravelersPerBooking: "Max {count} travelers per booking.",
      ageOutOfRange: "Age {age} is outside the accepted range for this product.",
      dependencyRequires: "{dependent} requires at least one {master}.",
      dependencyExcludes: "{dependent} can't be combined with {master}.",
      dependencyLimitPerMaster: "At most {limit} {dependent} per {master}.",
      dependencyLimitSum: "At most {limit} {dependent} allowed.",
      invalidEmail: "Enter a valid email address.",
    },
    warnings: {
      phoneMissing: "Phone number not set — useful for last-minute supplier contact.",
      billingCountryMissing: "Add a billing country so taxes can be calculated.",
      vatMissing: "Tax ID not set — needed for company (B2B) invoicing.",
      travelerFieldRequired: "{traveler}: {field} is required.",
      paymentIntentMissing: "No payment option chosen — this booking will be put on hold.",
      noTravelers: "Add at least one traveler.",
    },
    unsatisfied: {
      title: "Complete these details before continuing:",
      fallback: "This booking is missing a required detail. Review your selection and try again.",
      reasons: {
        pax_band_below_min: "Add more {band} travelers — this booking is below the minimum.",
        pax_band_above_max: "Remove some {band} travelers — this booking is over the maximum.",
        pax_total_below_min: "Add more travelers — this booking is below the minimum party size.",
        pax_total_above_max: "Remove some travelers — this booking is over the maximum party size.",
        pax_band_master_required: "{dependent} requires at least one {master}.",
        pax_band_excluded: "{dependent} can't be combined with {master}.",
        pax_band_per_master_exceeded:
          "Too many {dependent} for the number of {master} in this booking.",
        pax_band_sum_exceeded: "Too many {dependent} in this booking.",
        departure_required: "Choose a departure.",
        option_units_required: "Choose at least one room.",
        cabin_category_required: "Choose a cabin category.",
        cabin_number_required: "Choose a cabin number.",
        date_range_required: "Choose a check-in and a check-out date.",
        date_range_too_short: "This stay is shorter than the product allows.",
        date_range_too_long: "This stay is longer than the product allows.",
        occupancy_required: "Add at least one traveler.",
        air_arrangement_required: "Choose an air arrangement.",
        traveler_field_required: "Traveler {traveler}: {field} is required.",
        booking_field_required: "{field} is required.",
      },
    },
    configure: {
      travelers: "Travelers",
      departureDate: "Departure date",
      option: "Option",
      timeOptional: "Time (optional)",
      checkIn: "Check-in",
      checkOutWithNights: "Check-out ({minNights}-{maxNights} nights)",
      cabinCategory: "Cabin category",
      cabinNumber: "Cabin number",
      airArrangements: "Air arrangements",
      airOptions: {
        cruise_line: {
          label: "Cruise-line-arranged flights",
          description:
            "The cruise line books your flights in a coordinated package. Operator follows up with the air desk.",
        },
        independent: {
          label: "Independent flights",
          description: "Book flights yourself or via a separate flight booking line.",
        },
        none: {
          label: "No flights needed",
          description: "Regional cruise / driving to the port.",
        },
      },
      ageHintRange: "{min}-{max}y",
      ageHintMinimum: "{min}y+",
      ageHintMaximum: "up to {max}y",
    },
    billing: {
      title: "Billing & lead contact",
      buyerType: "Buyer type",
      individual: "Individual (B2C)",
      company: "Company (B2B)",
      firstName: "First name",
      lastName: "Last name",
      email: "Email",
      phone: "Phone",
      addressLine1: "Address line 1",
      addressLine2Optional: "Address line 2 (optional)",
      city: "City",
      region: "County / region",
      postalCode: "Postal code",
      country: "Country",
      companyName: "Company name",
      vatId: "Tax ID",
      leadContactSummaryNote: "Details come from the selected contact — edit them above.",
      leadContactSummaryEmpty: "Search and select a contact above, or create a new one.",
    },
    travelers: {
      title: "Travelers",
      partySize: "Party size",
      details: "Traveler details",
      decrease: "Decrease {label}",
      increase: "Increase {label}",
      empty: "Pick traveler counts in the Configure step to start adding details.",
      addTraveler: "Add another traveler",
      travelerType: "Traveler type",
      travelerNumber: "Traveler {number}",
      ageLabel: "age {age}",
      copyFromBilling: "Copy from billing",
      remove: "Remove",
    },
    accommodation: {
      title: "Accommodation",
      empty: "No accommodation options for this product.",
      extensionsAvailable: "{count} extension option{plural} available",
      ratePlan: "Rate plan",
      cancellationPrefix: "Cancellation:",
      includesPrefix: "Includes:",
    },
    addons: {
      title: "Extras",
      empty: "No extras available for this product.",
      otherBucket: "Other",
    },
    ancillaries: {
      decisionLegend: "Choose one option to continue",
      declineLabel: "No thanks, continue without this",
      declineDescription: "Nothing will be added to your booking.",
      providedBy: "Provided by {provider}",
      perPerson: "{price} per traveler",
      perBooking: "{price} for this booking",
      priceHeldUntil: "This price is held until {until}.",
      detailsToggle: "What's included",
      notAvailable: "Not available for this booking",
      documentsTitle: "Read before you buy",
      documentOpensInNewTab: "Opens in a new tab",
      acknowledgeLabel: "I have read {document}",
      acknowledgeRequired: "Confirm you have read the documents listed above before continuing.",
      multipleProvidersTitle: "Offers from {count} providers",
      multipleProvidersHint:
        "Compare the price and what each one includes. The list is not ranked in your favour or ours.",
      sourceUnavailableTitle: "Some providers could not be reached",
      sourceUnavailableBody: "The offers below are the ones that answered in time.",
      travelerFieldsTitle: "Details the provider needs",
      travelerFieldsHint:
        "These are sent to the provider you chose so they can complete the purchase.",
      travelerNumber: "Traveler {number}",
      sensitiveHint: "Kept only for this purchase.",
    },
    payment: {
      title: "Payment",
      empty: "No payment methods are available for this booking.",
      redirectedAfterConfirm:
        "You'll be redirected to our secure payment page after confirming the booking.",
      linkSentAfterConfirm:
        "After you confirm, the customer gets a secure link to pay. Nothing is charged yet.",
      cardOperatorLabel: "Online payment link",
      cardOperatorDescription:
        "The customer pays online via a secure link after you confirm. Nothing is charged instantly; inventory is reserved on confirmation.",
      generateLinkLabel: "Generate an online payment link",
      generateLinkHint:
        "Sends a secure payment link after you confirm. Leave off to reserve now and collect later.",
      inquiryNotice:
        "We'll send your details to the operator without locking inventory or taking payment. They'll get back to you with availability and a quote — typically within one business day.",
      bankTransferInstructions: "Bank transfer instructions",
      bankTransferDefaultNote:
        "After you submit, you'll receive an email with our bank details and a payment reference. Inventory is held pending payment.",
      intentLabels: {
        card: "Pay by card",
        bank_transfer: "Bank transfer",
        hold: "Hold for now",
        ticket_on_credit: "Agency credit account",
        inquiry: "Send as inquiry",
      },
      intentDescriptions: {
        card: "Charged immediately. Inventory is reserved on confirmation.",
        bank_transfer:
          "We'll send you bank details and a reference. Inventory is held while we wait for the transfer.",
        hold: "Reserve inventory without paying. The operator follows up to collect payment.",
        ticket_on_credit: "Charge to the agency's credit account.",
        inquiry:
          "No payment, no inventory hold. The operator gets back to you with availability and a quote.",
      },
    },
    review: {
      title: "Review & confirm",
      leadContact: "Lead contact",
      travelers: "Travelers",
      customerNotes: "Notes",
      customerNotesPlaceholder:
        "Anything we should know? (allergies, accessibility needs, special occasion...)",
      internalNotes: "Internal notes (operator-only)",
      confirmBooking: "Confirm booking",
      confirming: "Confirming...",
      completeToConfirm: "Complete the highlighted sections above to confirm.",
      priceOverrideToggle: "Override total price",
      priceOverrideAmount: "Total price",
      priceOverrideReason: "Override reason",
      priceOverrideReasonPlaceholder: "Why is the price being changed?",
      priceOverrideReasonRequired: "A reason is required when the price is changed.",
    },
    documents: {
      saveAsDraft: "Save as draft",
      saveAsDraftHint:
        "Keep the booking as a draft. Leave unchecked to create it live — confirmed if the payment is marked paid, otherwise awaiting payment.",
    },
    contract: {
      defaultTitle: "Booking contract",
      description:
        "Please review the contract below. You can scroll through the document, then tick the boxes and click Accept to continue.",
      errorPrefix: "Couldn't load the contract preview:",
      iframeTitle: "{name} preview",
      termsLabel:
        "I have read and agree to the terms of this contract. I understand that this booking is binding once accepted.",
      marketingLabel:
        "I'd like to receive occasional travel offers and updates by email. (You can unsubscribe at any time.)",
      cancel: "Cancel",
      acceptAndContinue: "Accept and continue",
      previewRequestFailed: "Preview request failed: {status}",
      previewMissing: "Preview response missing rendered body or template metadata",
    },
    sidePanel: {
      youAreBooking: "You're booking",
      total: "Total",
      pricingHint: "Add travelers to see pricing.",
      pricingHintRooms: "Select rooms to see pricing.",
      guestSingular: "traveler",
      guestPlural: "travelers",
      filledOf: "{filled} of {total} filled",
      roomSingular: "room",
      roomPlural: "rooms",
      addOnSingular: "extra",
      addOnPlural: "extras",
      card: "Card",
      hold: "Hold",
      onCredit: "On credit",
      confirmAndBook: "Confirm and book",
      reviewDetails: "Review your details and confirm to book.",
      noTravelersYet: "No travelers yet.",
      notSelected: "Not selected.",
      noAddonsSelected: "No extras selected.",
      adults: "Adults",
      children: "Children",
      infants: "Infants",
      departure: "Departure",
      date: "Date",
      checkIn: "Check-in",
      checkOut: "Check-out",
      cabin: "Cabin",
      name: "Name",
      email: "Email",
      phone: "Phone",
      buyer: "Buyer",
      company: "Company",
      individual: "Individual",
      vat: "Tax ID",
      address: "Address",
      travelerNumber: "Traveler {number}",
      dob: "DOB",
      method: "Method",
      schedule: "Schedule",
      payByCard: "Pay by card",
      ticketOnCredit: "Ticket on credit",
      holdNoChargeYet: "Hold (no charge yet)",
    },
  },
} satisfies BookingsUiJourneyMessages

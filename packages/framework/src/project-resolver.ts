  ...
  const files: FrameworkGeneratedProjectFile[] = [
    ...
    },
    {
      ...
      contents: buildProjectMigrationRunnerModule({
        migrationPlan,
        runtimeEntryOverrides,
        migrationResolutionRoots: relativeMigrationResolutionRoots(
          projectRoot,
          migrationRunner,
          materialized.resolutionRoots,
        ),
...
/**
 * Lower the build-time resolution roots to paths relative to the generated
 * migration runner's directory. The runner reconstructs them to absolute paths
 * from its own `import.meta.url`, so the layout stays correct wherever the
 * `.voyant` output is executed as long as the roots keep their relative position
 * (they do: `voyant build` and `voyant migrate` share the same output tree).
 */
function relativeMigrationResolutionRoots(
  projectRoot: string,
  migrationRunner: string,
  resolutionRoots: readonly string[],
): string[] {
  const runnerDirectory = path.dirname(path.join(projectRoot, ".voyant", migrationRunner))
  return resolutionRoots.map((root) => {
    const relative = path.relative(runnerDirectory, root).replaceAll("\\", "/")
...
export function buildProjectMigrationRunnerModule(input: {
  ...
  /**
   * Package-resolution roots (relative to the generated runner's directory) that
   * the build-time resolver used to locate schema-owning migration packages.
   * Schema packages owned by a sealed product distribution (the operator-standard
   * BOM) are TRANSITIVE dependencies nested under that distribution's own
   * `node_modules`, so they are not resolvable from the operator root alone. The
   * runner reconstructs these to absolute paths from its own `import.meta.url` and
   * threads them to the migration source loader so every package resolves from the
   * same nesting the build did.
   */
  migrationResolutionRoots?: readonly string[]
}): string {

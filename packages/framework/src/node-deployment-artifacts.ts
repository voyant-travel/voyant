// agent-quality: file-size exception -- owner: framework; graph artifact parsing and pre-boot validation stay co-located so every Node host uses one trusted generated deployment contract reader.
...
export interface VoyantNodeDeploymentGraphArtifactSummary {
  graphHash: string
  ...
  migrationSources: readonly VoyantNodeDeploymentGraphMigrationSource[]
}

export interface VoyantNodeDeploymentGraphScheduledJob {
  id: string
  cron: string
  description: string
  route: string
...
export interface VoyantNodeDeploymentGraphResourceRequirement {
  resourceKey: string
  roles: readonly string[]
  provider: string
  required: boolean
  env: readonly VoyantNodeDeploymentGraphEnvRequirement[]
  notes?: string
}

export interface VoyantNodeDeploymentGraphMigrationSource {
  packageName: string
  schema: string
}

export interface VoyantNodeDeploymentGraphEnvRequirement {
  name: string
...
export type VoyantNodeDeploymentGraphEnv = Record<string, unknown>
...
export function loadVoyantNodeDeploymentGraphArtifacts(
  baseUrl: string,
  ...
  if (target !== "node") {
    throw new Error(`Node deployment graph target must be node, got ${String(target)}`)
  }
  ...
  if (!hasExpectedNodeRuntimeEntry) {
    throw new Error(
      `deployment artifacts must include the framework Node runtime entry ${EXPECTED_NODE_RUNTIME_ENTRY_ID}`,
    )
  }
...
export function validateVoyantNodeDeploymentGraphResourceEnv(
  summary: Pick<VoyantNodeDeploymentGraphArtifactSummary, "resourceRequirements">,
  env: VoyantNodeDeploymentGraphEnv,
): string[] {
  const issues: string[] = []
...
export function assertVoyantNodeDeploymentGraphResourceEnv(
  summary: Pick<VoyantNodeDeploymentGraphArtifactSummary, "resourceRequirements">,
  env: VoyantNodeDeploymentGraphEnv,
): void {
  const issues = validateVoyantNodeDeploymentGraphResourceEnv(summary, env)
...
function collectScheduledJobs(value: unknown): VoyantNodeDeploymentGraphScheduledJob[] {
  if (value == null) {
    throw new Error(
...
function collectMigrationSources(
  value: unknown,
  packageNames: readonly string[],
): VoyantNodeDeploymentGraphMigrationSource[] {
...
function collectResourceRequirements(
  value: unknown,
): VoyantNodeDeploymentGraphResourceRequirement[] {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
...
function requireEnvFormat(
  value: unknown,
  label: string,
): NonNullable<VoyantNodeDeploymentGraphEnvRequirement["format"]> {
...
function validateGeneratedRuntimeEntrySource(input: {
  ...
  summary: VoyantNodeDeploymentGraphArtifactSummary
  target: string
}): void {

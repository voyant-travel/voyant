...
function TypographyH3({ className, ...props }: React.ComponentProps<"h3">) {
  return (
    <h3
      className={cn("scroll-m-20 font-heading text-2xl font-semibold tracking-tight", className)}
      {...props}
    />
  )
}
...
function TypographyUl({ className, ...props }: React.ComponentProps<"ul">) {
  return <ul className={cn("my-6 ml-6 list-disc [&>li]:mt-2", className)} {...props} />
}

function TypographyOl({ className, ...props }: React.ComponentProps<"ol">) {
...
function TypographyTableWrapper({ className, ...props }: React.ComponentProps<"div">) {

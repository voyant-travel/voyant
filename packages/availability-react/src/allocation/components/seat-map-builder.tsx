    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col items-start gap-1.5 overflow-x-auto rounded-md border bg-muted/10 p-3">
        {spec.rows.map((row, rowIndex) => (
          <div
            // biome-ignore lint/suspicious/noArrayIndexKey: rows are positional by design; row 1 is always the first row of the bus. -- owner: availability-react; existing suppression is intentional pending typed cleanup.
            key={`row-${rowIndex}`}
          ...
          >
            ...
            {row.cells.map((cell, cellIndex) => (
              <button
                // biome-ignore lint/suspicious/noArrayIndexKey: cells are positional within a row. -- owner: availability-react; existing suppression is intentional pending typed cleanup.

...
primitives and page-level compositions (formerly `@voyantjs/finance-ui`).
...
```bash
pnpm add @voyantjs/finance-react @voyantjs/ui @tanstack/react-query react react-dom
```

`@voyantjs/ui` provides the design-system primitives (required only for the
...
```tsx
import { FinanceUiMessagesProvider } from "@voyantjs/finance-react/ui"
import { financeUiEn } from "@voyantjs/finance-react/i18n/en"
import { financeUiRo } from "@voyantjs/finance-react/i18n/ro"
```

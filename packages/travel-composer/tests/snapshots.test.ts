...
import { TravelComposerInvariantError, travelComposerService } from "../src/service.js"
...
describe("travel composer trip snapshots", () => {
  ...
  it("rejects snapshots when active components have no pricing snapshot", async () => {
    ...
    await expect(
      travelComposerService.freezeTripSnapshot(db, { envelopeId: "trip_123" }),
    ).rejects.toThrowError(TravelComposerInvariantError)
  })
})

...
import { createTravelComposerRoutes } from "../src/routes.js"

describe("travel composer routes", () => {
  it("exposes package health", async () => {
    const app = createTravelComposerRoutes()
  ...
  it("keeps adapter-backed operations unavailable until runtime deps are configured", async () => {
    ...
    await expect(res.json()).resolves.toEqual({
      error: "Travel composer checkout dependencies are not configured",
    })
  })

  it("blocks admin-only mutation routes on the public surface", async () => {
    const app = createTravelComposerRoutes({ surface: "public" })
    ...
    await expect(res.json()).resolves.toEqual({
      error: "Travel composer operation is admin-only",
    })
  })

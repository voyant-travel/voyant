      ...
      {aboutSections.length > 0 && (
        <div className="mt-10">
          ...
          <div className="columns-1 gap-4 sm:columns-2 lg:columns-3 [&>*]:mb-4 [&>*]:break-inside-avoid">
            {aboutSections.map((s) => {
              const Icon = (s.kind ? SECTION_ICONS[s.kind] : undefined) ?? Info
              return (
...
function boardLabel(board: string | null, t: DetailMessages): string {
...
function roomCodeOf(roomTypeId: string | null): string | null {
...
function formatStars(value: number | null | undefined): string | null {
  if (value == null || !Number.isFinite(value) || value <= 0) return null
  return `${Number.isInteger(value) ? value : value.toFixed(1)}★`
}

function formatCountry(code: string | null | undefined, locale?: string): string | null {

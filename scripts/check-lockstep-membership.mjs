 ...
 * release.runtime-packages.generated.json. Every member must be explicitly
 * selected by starters/operator/voyant.config.ts. It is the set that should version in lockstep
 ...
 *   1. verify every lockstep member is selected by voyant.config.ts `modules`;
...
/** Extract the `modules: [ ... ]` block from voyant.config.ts and pull the
 * quoted package names from it (ignoring // comments). */
function readMountedModules() {
  ...
  if (end === -1) throw new Error("voyant.config.ts: unterminated `modules` array")
...
for (const name of runtimeSet) {
  ...
  if (!mounted.includes(name)) {
    violations.push(`lockstep runtime package "${name}" is not selected by voyant.config.ts`)
  }
}
...
const manifestBody = `${JSON.stringify(
  {
    $comment:
      "GENERATED by scripts/check-lockstep-membership.mjs — do not edit. Lockstep runtime packages must be explicitly selected by voyant.config.ts. Refresh with `node scripts/check-lockstep-membership.mjs --emit`.",
...
if (EMIT) {
  ...
  if (current !== manifestBody) {
    violations.push(
      "release.runtime-packages.generated.json is stale (voyant.config modules changed) — run `node scripts/check-lockstep-membership.mjs --emit`",
    )
  }
}

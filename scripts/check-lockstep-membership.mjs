 ...
 *
 * The "runtime modules" are exactly the packages mounted by
 * starters/operator/voyant.config.ts `modules` — the backend Hono modules a
 * deployment runs. They are the set that should eventually version in lockstep
 ...
 * *-contracts, plugins, infra, apps, tooling) and deliberately excludes
 * `additionalSchemas` (migrated-but-not-mounted optional verticals).
 *
 * This checker is ADDITIVE — it does NOT flip the changeset `fixed` groups:
 *   1. derive the runtime set from voyant.config.ts `modules`;
 *   2. emit/verify `release.runtime-packages.generated.json` (committed source
 *      of truth; `--emit` rewrites it, default mode fails on drift);
...
for (const name of mounted) {
  if (!pkgs.has(name)) {
    violations.push(`voyant.config modules: "${name}" is not a workspace package under packages/`)
  }
}
...
const runtimeSet = mounted.filter((n) => pkgs.has(n))

// (2) emit / verify the committed manifest.
const manifestBody = `${JSON.stringify(
  {
    $comment:
      "GENERATED by scripts/check-lockstep-membership.mjs — do not edit. Runtime modules mounted by voyant.config.ts `modules`; the lockstep set. Refresh with `node scripts/check-lockstep-membership.mjs --emit`.",

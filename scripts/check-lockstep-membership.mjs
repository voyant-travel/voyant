...
const frameworkManifest = JSON.parse(
  readFileSync(join(ROOT, "packages/framework/package.json"), "utf-8"),
)
const bomRuntimeAdapters = new Set(
  [...pkgs.entries()]
    .filter(([name, directory]) => {
      ...
      return (
        ...
        manifest.voyant?.schemaVersion === "voyant.package.v1" &&
        manifest.voyant?.kind === "library" &&
        manifest.voyant?.runtime?.entry === "./runtime-contributor" &&
        manifest.voyant?.compatibleWith?.targets?.includes("node")
      )
    })
    .map(([name]) => name),
)
...
for (const name of runtimeSet) {
  ...
  if (!mounted.includes(name) && !bomRuntimeAdapters.has(name)) {
    violations.push(
      `lockstep runtime package "${name}" is not selected by the standard Operator distribution or its Node adapter package records`,
    )
  }
}
...
const manifestBody = `${JSON.stringify(
  {
    $comment:
      "GENERATED by scripts/check-lockstep-membership.mjs — do not edit. Lockstep runtime packages must be selected by the framework-owned standard Operator distribution or its Node adapter package records. Refresh with `node scripts/check-lockstep-membership.mjs --emit`.",

import { submitBookingReservationPlan } from "@voyantjs/bookings/reservation-plans"
...
} from "./trip-composer-catalog-runtime"
import {
  reserveNonCatalogComponent,
  validateNonCatalogComponentBeforeReserve,
} from "./trip-composer-flight-runtime"
import { startTripCheckout } from "./trip-composer-trip-checkout"

export function createOperatorTripComposerRoutesOptions(): TripComposerRoutesOptions {
  return {
...
export const tripComposerPaymentBundle: HonoBundle = {
  name: "trip-composer-payment-completion",
  bootstrap: ({ bindings, eventBus }) => {
    ...
    eventBus.subscribe<PaymentCompletedEvent>("payment.completed", async ({ data }) => {
      ...
      try {
        ...
        console.error("[trip-composer] payment completion failed", err)
      }
    })
  },
}

...
import type { Trip, TripCheckoutInput, TripCheckoutResult } from "@voyantjs/trip-composer"
...
export async function startTripCheckout(
  ...
  if (input.intent !== "bank_transfer") {
    try {
      ...
      console.warn("[trip-composer] netopia start failed for trip payment session:", error)
    }
  }
...
async function quoteFx(
  ...
  const response = await fetch(url, {
    headers: {
      ...
      "x-voyant-sdk": "voyant-operator-trip-composer",
    },
  })

 ...
 * search active it falls back to the indexed browse grid.
 *
...
type SearchMessages = ReturnType<typeof useAdminMessages>["catalog"]["search"]
type BoardLabels = ReturnType<typeof useAdminMessages>["catalog"]["detail"]["boards"]
...
export function DynamicCatalogPage({ search, onSearchChange }: DynamicCatalogPageProps) {
  const messages = useAdminMessages()
  const nav = messages.nav
  const s = messages.catalog.search
  const boards = messages.catalog.detail.boards
  ...
  // Populate the destination picker from the indexed country facet.
  useEffect(() => {
    let cancelled = false
    void (async () => {
      try {
        const res = await fetch(`${getApiUrl()}/v1/admin/catalog/search`, {
          method: "POST",
        ...
        const json = (await res.json()) as {
          facets?: Record<string, Array<{ value: string | number }>>
        }
        if (cancelled) return
        const region = new Intl.DisplayNames(resolvedLocale, { type: "region" })
        const opts = (json.facets?.countryCodes ?? [])
        ...
        // Picker stays empty; the operator can still browse.
      }
    })()
    return () => {
      cancelled = true
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(`${getApiUrl()}/v1/admin/catalog/departure-airports`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ destination: { countryCode: destination } }),
        })
        const json = (await res.json()) as { departureAirports?: AirportOption[] }
        if (!cancelled) {
          setAirportOptions(Array.isArray(json.departureAirports) ? json.departureAirports : [])
        }
      } catch {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/v1/admin/catalog/package-search`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
      ...
      if (Array.isArray(json.departureAirports) && json.departureAirports.length > 0) {
  ...
  }, [destination, nights, adults, month])
  ...
  const openHoliday = (productId: string) => {
    const { href } = router.buildLocation({
      ...
      params: { productId },
      search: { adults, nights: Number(nights) || 7 },
    })
  ...
  return (
    <div className="mx-auto w-full max-w-screen-2xl px-6 py-6 lg:px-8">
      ...
      <div className="mb-4">
        <h1 className="font-semibold text-2xl">{nav.catalogProducts}</h1>
        <p className="text-muted-foreground text-sm">{nav.catalogProductsTagline}</p>
      </div>
      ...
      {state.status === "browse" ? (
        <div className="mt-4">
          <CatalogVerticalPage
            ...
            embedded
            lockedFacets={{ supplyModel: ["dynamic"] }}

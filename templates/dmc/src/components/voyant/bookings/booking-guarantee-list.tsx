...
export function BookingGuaranteeList({ bookingId }: BookingGuaranteeListProps) {
  ...
  const guaranteeMessages = detailMessages.guarantees
                ...
                {guarantees.map((g) => (
                  <tr key={g.id} className="border-b last:border-b-0">
                    ...
                    <td className="p-2 text-right font-mono">
                      {formatAmount(g.amountCents, g.currency, detailMessages.noValue)}
                    </td>
                    <td className="p-2">{g.provider ?? detailMessages.noValue}</td>
                    <td className="max-w-[150px] truncate p-2 font-mono text-xs">
                      {g.referenceNumber ?? detailMessages.noValue}
                    </td>
                    <td className="p-2">
                      {g.expiresAt

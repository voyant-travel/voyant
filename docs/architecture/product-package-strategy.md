...
| `@voyant-travel/workflows-node-step-container`, `@voyant-travel/workflows-orchestrator-worker`, `@voyant-travel/workflows-selfhost-cloudflare-worker`, `@voyant-travel/workflows-tenant-worker` | Keep as legacy workflow deployment apps. | These are compatibility targets for older workflow infrastructure and should remain outside travel-domain consolidation. Do not present them as the current managed Cloud runtime. |

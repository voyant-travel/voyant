...
| `@voyant-travel/workflows`, `@voyant-travel/workflows-react`, `@voyant-travel/workflow-runs`, `@voyant-travel/workflows-orchestrator`, `@voyant-travel/workflows-orchestrator-node` | Keep as workflow infrastructure family. | These are runtime/orchestration seams, not travel-domain seams. The retired Cloudflare adapter packages and external step-server artifact are removed because workflows are node-only. |

...
2. **§3 Queues** → producers fine from tenant workers; consumers must live in a platform-level
   worker (the orchestrator already exists) that dispatches back into the namespace, or DO
   alarms (`WorkflowRunDO` already proves the pattern in-template).
...
| # | Work item | Detail |
|---|---|---|
| 2.1 | **Transactional outbox.** `event_outbox` table written in the same transaction as the domain write; `emit` becomes "insert outbox row(s) + best-effort immediate drain via `waitUntil`"; a drain loop delivers to subscribers/workflow ingest with retries, per-event idempotency (stable event IDs already exist in the orchestrator driver), and dead-lettering. Drain trigger, best-first: the Node lane with `LISTEN/NOTIFY` on the direct connection (2.6 — push-based, near-real-time); DO alarm per environment where DO bindings exist; platform scheduler-dispatch loop (already running for workflow schedules) as the lowest-dependency fallback. | Events become durable; subscribers become genuinely async; crash ≠ silent loss. This is the keystone for everything async. |

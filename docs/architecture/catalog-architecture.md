...
External and operator-built adapters must execute `assertIndexerAdapterConformance` from `@voyant-travel/catalog-contracts/indexer/conformance` in their own test suites. The runner verifies non-empty keyword matching, replacement by document id, hit fidelity, slice isolation, ordered sorting, pagination limits and terminal cursors, filters, exact facet buckets, bulk reindexing, deletion, and optional admin behavior without imposing a test framework. Facet limits are bounded by the portable `MAX_FACET_BUCKETS` value of 250; omitted and larger limits use that maximum. Declared semantic search must ignore query text for matching and rank by vector similarity. Hybrid search must retain the union of keyword-only and vector-only candidates, expose larger-is-better scores, and honor `alpha`; cross-audience federation must preserve that score ordering. Vector-aware admin scans must preserve every fixture's exact embeddings and model ids. These are portable observable behaviors. Provider-specific query topology, native federation, and proprietary ranking quality belong in provider-owned tests. Hosted engines use the runner's `settle` hook for eventual consistency.

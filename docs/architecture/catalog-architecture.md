...
External and operator-built adapters must execute `assertIndexerAdapterConformance` from `@voyant-travel/catalog-contracts/indexer/conformance` in their own test suites. The runner verifies non-empty keyword matching, replacement by document id, hit fidelity, slice isolation, sorting, pagination limits and terminal cursors, filters, facet counts and limits, bulk reindexing, deletion, and optional admin behavior without imposing a test framework. Declared vector, hybrid, native cross-audience federation, and admin-denormalization capabilities each trigger a focused portable request; the runner verifies request handling and result fidelity, not provider-specific ranking quality. Hosted engines use the runner's `settle` hook for eventual consistency.

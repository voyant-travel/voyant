...
External and operator-built adapters must execute `assertIndexerAdapterConformance` from `@voyant-travel/catalog-contracts/indexer/conformance` in their own test suites. The runner verifies non-empty keyword matching, replacement by document id, hit fidelity, slice isolation, ordered sorting, pagination limits and terminal cursors, filters, facet counts and limits, bulk reindexing, deletion, and optional admin behavior without imposing a test framework. Declared vector and hybrid capabilities must rank competing similarity signals and honor `alpha`; cross-audience federation is certified by the semantic result of one `adapter.search(...)` call; and vector-aware admin scans must preserve fixture embeddings and model ids. These are portable observable behaviors. Provider-specific query topology, native federation, and proprietary ranking quality belong in provider-owned tests. Hosted engines use the runner's `settle` hook for eventual consistency.

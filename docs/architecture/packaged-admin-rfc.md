...
- *Zero-prop components only.* `AdminUiRouteContribution.component` attaches
  ...
  thin files delete themselves once the factory builds the tree. The first
  route-assembly increment (§4.8) already generates the zero-prop thin files
...
### 4.8 Route assembly, increment 1: generated thin route files

The §4.2 endgame replaces file-based routing with a code-based tree assembled
by `createAdminApp`. The first increment gets there without touching the
host's router: instead of *removing* the thin route files, the CLI now
*generates* them.

**`voyant admin generate --routes`.** The command statically scans each
resolved admin entry's source (the same no-execution discipline as the
composition generator and the doctor — package.json + source text only),
extracting the full contribution shape: id, path (including
option-destructuring defaults and template-literal paths), component/loader/
`validateSearch` presence, and the `ssr`/`preload` literals. For every
**zero-prop** contribution — component present, no `$param` segments, per the
§4.2 finding above — it emits a thin host into the host's file-based tree
(default `src/routes/_workspace`, configurable via the manifest's
`admin.routes.dir`). The emitted file resolves the contribution through
`requireAdminRoute(create<Domain>AdminExtension(), "<route-id>")` and binds
the app runtime exactly as the hand-written hosts do
(`{ baseUrl: getApiUrl(), fetcher: operatorFetcher }`; module and export
names configurable via `admin.routes` in `voyant.config.ts`). `--check` is
the CI drift gate, same as the composition file's.

**Ejection semantics.** Every generated file opens with the
`// GENERATED by voyant admin generate --routes` header. The generator never
overwrites a file that lacks it — deleting the header (or hand-writing the
file in the first place) IS the ejection act: explicit, greppable, and
reported as a skip on every run. This is §2's principle 3 applied to route
binding: overriding a generated host is a visible decision, not silent
divergence. Param-taking detail hosts remain hand-written by rule, not by
ejection — the generator skips them and says so.

**Doctor Finding D.** The §4.7 endgame's parity half landed with this
increment: `voyant admin doctor` now statically compares the
`AdminDestinations` keys each mounted admin entry declares (the
`declare module "@voyantjs/admin"` blocks) against the host's resolver map
(the object marked `satisfies AdminDestinationResolvers`, default
`src/lib/admin-destinations.ts`). Both directions are reported — a declared
key with no resolver, and a resolver matching no declared key. Report-only,
like Findings A–C; the typecheck (`satisfies` exhaustiveness) remains the
hard gate, the doctor catches the inverse direction and hosts whose
...
**Remaining horizon.** Two steps left to the §4.2 endgame:

1. **Generated typed route tree.** Replace the generated *files* with a
   generated *tree*: one build step emits code-based route definitions for
   all extension routes (param-taking included — the route file's
   `useParams` binding is mechanical once the tree is generated, which
   dissolves the zero-prop restriction) and the `Register` augmentation that
   restores fully typed `Link`s across packages. File-based routing then
   remains only for genuinely custom host pages.
2. **Generated resolver maps.** Route-backed destinations are derivable from
   the same scan — a destination key whose target is an extension route can
   have its resolver emitted rather than hand-written, shrinking the
   hand-maintained map to genuinely custom targets. Finding D then flips
   from report to gate for the generated portion.

The ejection contract carries forward unchanged through both steps: a
hand-written file (or resolver) without the generated marker is ejected, and
the generator works around it.

...
- **Shipped:**
  ...
  - The live operator runner (`starters/operator/scripts/migrate.ts`) rewritten to discover package sources from `drizzle.schemas.generated.ts`, topo-order by `requiresSchemas`, and apply via `applyMigrations`; the framework bundle is no longer loaded on the apply path — PR #2018 (3b-2).

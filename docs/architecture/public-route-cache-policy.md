...
| Storefront settings, departure browse/detail, itinerary, product extensions, availability snapshots, offer reads | `shared-response-cache` | Checkout and customer mutations remain live. |
| Catalog sourced content for products, cruises, and accommodations | `shared-response-cache`, `kv-read-model` | Public mounts pass a cache policy; admin mounts leave it unset. |

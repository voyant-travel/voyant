...
- `voyant db generate` (and a standalone `voyant db schemas --emit`) runs
  the manifest resolver and writes
...
Template-local schema entrypoints are declared in `voyant.config.ts` as
`schemas: ["./src/db/schema.ts"]` and are appended after the package-derived
closure. This keeps deployment-owned tables visible to generation without
pretending they belong to a reusable package.
...
Remove the mtime-ordering footgun (§1.4) by using both:

- **timestamp-prefixed** migration names (e.g. `YYYYMMDDHHMM_*`) so ordering is
  total and machine-independent; and
- adding a lint (part of `doctor`) that **rejects duplicate `NNNN_` prefixes**,
  forcing legacy collisions to be cleaned up instead of hidden by mtime.

`voyant db generate` is the blessed path for composed deployment roots and
defaults to `--prefix timestamp` unless the caller supplies an explicit
`--prefix`. Raw `drizzle-kit generate` should only be used for package-owned
standalone schema work or with the same timestamp prefix and generated-manifest
refresh steps. Historical duplicate prefixes that cannot be safely renamed are
captured in `migrations/duplicate-prefixes.baseline.json`; `doctor` warns on any
duplicate group that is not exactly present in that baseline.
...
| Command | Change |
|---|---|
| `db generate` | before proxying drizzle-kit: regenerate `drizzle.links.generated.ts` from link definitions, regenerate `drizzle.schemas.generated.ts` from the manifest, default to `--prefix timestamp`, then let drizzle-kit diff the generated Drizzle table definitions (§4.2, §4.4, §4.6) |
...
- Switch composed deployment `drizzle.config.ts` files (`templates/dmc`,
  `templates/operator`, `apps/dev`) to import it **only after** generated output
  matches the hand lists exactly. Standalone package-local configs, such as
  `packages/catalog/drizzle.config.ts`, stay package-owned until they have their
  own manifest story.
...
- If `drizzle-kit generate` prompts for unrelated rename/conflict decisions,
  resolve that snapshot drift separately instead of accepting prompts blindly.
...
- Extension subpath entries seed the owning package's schema closure unless a
  later per-extension schema declaration is needed.
...
- Adopt deterministic migration ordering: `voyant db generate` defaults to
  timestamp prefixes, and `doctor` warns on duplicate prefixes unless the exact
  legacy collision is baselined; document the rule in `schema-discipline.md`.
...
- **Timestamp-prefixed migration generation is the composed-root default.**
  `voyant db generate` passes `--prefix timestamp` unless the caller explicitly
  chooses another prefix, while `doctor` reports new/unbaselined duplicate
  prefixes.
...
4. **Legacy prefix cleanup policy** — exact duplicate-prefix baselines keep
   already-published migration names immutable; decide whether cleanup ever
   happens through a dedicated migration-history normalization PR.
...
- **Deployment `drizzle.config.ts` files contain no hand-curated schema paths** —
  composed roots consume the generated manifest, and they cannot drift.

...
Voyant custom storefronts consume booking behavior through the framework-agnostic
`@voyant-travel/storefront-sdk` package. They do not assemble a checkout from
...
- `@voyant-travel/storefront` for public discovery, availability, and price preview
...
belong in `@voyant-travel/storefront-react` or a consumer application.
...
supplier, and fulfillment state stays on the aggregate that owns it; storefronts
...
Use `previewStorefrontDeparturePrice(...)` for discovery-time price cards. A
Booking Session Quote remains authoritative for commit. Storefronts must not
copy price-preview results into low-level Booking create payloads.

## Errors

`VoyantStorefrontApiError` exposes `normalizedError` for structured public-route
...
The SDK is not a hosted storefront, UI component library, persistence layer, or

# ADR 0019: Channel-authoritative Product distribution
...
Distribution already models named Channels and active Product–Channel
assignments. The duplicate Product flags allow contradictory states and cannot
express the normal requirement “sell this Product on our storefront and one OTA,
but not on another OTA”.
...
2. An active `channel_product_mappings` row joined to an active Channel is the
   distribution authority for that Channel.
3. The operator's own website/storefront is modeled as a Channel. There is no
   implicit “site” publication switch.
4. Channel-scoped catalog slices require an active mapping to that exact
   Channel. The legacy unchannelled customer slice requires at least one active
   mapping and acts only as an aggregate compatibility slice.
...
6. The lifecycle tools are status transitions: publish → `active`, unpublish →
   ...
   Channel-assignment operation, not an unpublish operation.

Staff catalog slices continue to show active Products independently of Channel
assignment so operators can find and configure undistributed inventory.
...
1. create an active Channel for every direct storefront or other sales surface;
2. create active Product mappings for the surfaces where each Product belongs;
3. run a catalog reindex so deprecated Product flags disappear from documents
   and Channel-scoped slices reflect the mappings; and
4. migrate API clients away from writing or filtering by `visibility` and
   `activated`.

## Consequences

- A Product can remain active while being removed from one Channel.
- A newly active Product with no active Channel assignment is intentionally not
  customer/partner discoverable.
- Mapping create/update/delete events already trigger Product publication
  reindexing, so Channel changes update discovery without editing the Product.

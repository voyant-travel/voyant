...
| **Channel**       | A named sales surface or distribution counterparty selling our inventory (the operator's direct storefront, OTA, affiliate, reseller, marketplace, or API partner). An active Product–Channel assignment is the authority for whether an active Product is distributed there. | *partner, distributor, visible on site*    |
...
| **Product–Channel assignment** | The active mapping that publishes an active Product to one Channel. Removing or deactivating it removes that Product from the Channel without changing the Product's lifecycle status or its other Channel assignments. | *visible on site, product visibility* |
